...
这是最简单的使用方式，无需手动拉取代码或配置环境。脚本会自动拉取最新 `dev` 分支代码到 `~/.agent-vibes` 隐藏目录，并安装依赖启动服务。

**🖥️ Windows (PowerShell):**

```powershell
# 默认安装到 C:\Users\<用户名>\.agent-vibes
...
若未经定制修改，你可以直接从 [GitHub Releases](https://github.com/funny-vibes/agent-vibes/releases) 一键下载并安装：
